...
class PylonSizing():
    ...
    def I_xx(self, x): return np.pi/4 *  (x[1]**4 - x[0]**4)

    def get_area(self, x):
        return np.pi*(x[1]**2 - x[0]**2)

    def weight_func(self, x):
        return np.pi*(x[1]**2 - x[0]**2)*const.rho_material*self.L
    ...
    def  optimize_pylon_sizing(self, x0):
        ...
        cons = ({'type': 'ineq', 'fun': self.column_buckling_constraint },
                {'type': 'ineq', 'fun': self.von_mises_constraint },
                {'type': 'ineq', 'fun': self.r2_larger_than_r1 },
                {'type': 'ineq', 'fun': self.eigenfreq_constraint}
                )
        bnds = ((0.04, 0.3), (0.02,0.35))
