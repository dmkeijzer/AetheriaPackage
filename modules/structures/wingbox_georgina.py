...
class Wingbox():
    ...
    def get_y_rib_loc(self):
        ...
        y_rib_3 = np.array([self.y_rotor_loc[1] - 0.5 * self.nacelle_w])
        y_rib_2 = np.array(y_rib_3 - 0.15)
        ...
        y_rib_loc = np.concatenate((y_rib_0, y_rib_1 ,y_rib_2, y_rib_3,y_rib_sec0,y_rib_sec1))
    ...
    def I_xx(self,t_sp,h_st,t_st,w_st,tsk):
        ...
        i = lambda z: 2 * (Ist + A * (0.5 * h(z)) ** 2) * self.n_st + 2 * Isp(z) + 2 * (0.6 * self.chord_root * tsk ** 3 / 12 + self.t_sk * 0.6 * self.chord_root * (0.5 * h(z)) ** 2)
        return i
    ...
    def t_arr(self,tmax, tmin):
        return np.linspace(tmax,tmin, self.n_ribs_sec0 + self.n_ribs_sec1 + 4 + 1)
    ...
    def N_xy(self, t_sp, t_rib, h_st,t_st,w_st,tmax,tmin):
        ...
        tarr = self.t_arr(tmax,tmin)
        sta = self.get_y_rib_loc()
        Vz= self.shear_force(t_sp, t_rib, h_st,t_st,w_st)
        Nxy = np.zeros(len(tarr))
    ...
    def f_ult(self,h_st,t_st,w_st,tmax,tmin):
        ...
        # n=n_st(c_r,b_st)
        tarr=self.t_arr(tmax,tmin)
        c=self.chord(Wingbox)
        h=self.height(Wingbox)
        stations= self.get_y_rib_loc() #FIXME change this to an input 
        f_uts=np.zeros(len(tarr))
        for i in range(len(tarr)):
            A=self.n_max*A_st+0.6*c(stations[i])*tarr
    ...
    def post_buckling(self, t_sp, t_rib, h_st,t_st,w_st, tmax,tmin):
        ...
        px= self.n_max*self.shear_force(t_sp, t_rib, h_st,t_st,w_st)
...
class Wingbox_optimization(om.ExplicitComponent):
    ...
    def compute(self, inputs, outputs):
        ...
        self.WingboxClass.post_buckling(  tsp, trib,   bst, hst, tst, wst, t_max, t_min),
        self.WingboxClass.von_Mises(  tsp, trib,  bst, hst, tst,wst,t_max, t_min),
