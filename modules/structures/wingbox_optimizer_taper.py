...
class Wingbox():
    ...
    def chord(self,y,x):
        t_sp, h_st, t_st, t_sk, t_tb, taper = x
        return self.chord_root - self.chord_root * (1 - taper) * y * 2 / self.span

    def height(self,y,x):
        return self.thickness_to_chord * self.chord(y,x)
    
    def l_sk(self,y,x):
        return np.sqrt(self.height(y,x) ** 2 + (0.25 * self.chord(y,x)) ** 2)
    ...
    def w_sp(self,y,x):
        t_sp, h_st, t_st, t_sk, t_tb, taper = x
        return 0.3 * self.height(y,x)




    def I_sp_x(self,t_sp,y,x):
        t_sp, h_st, t_st, t_sk, t_tb, taper = x
        h = self.height(y,x)
        wsp = self.w_sp(y,x)
    ...
    def get_x_le(self,y,x):
        return self.x_lewing + 0.25*self.chord_root - 0.25*self.chord(y,x)

    def get_x_te(self,y,x):
        return self.x_lewing + 0.25*self.chord_root +0.75*self.chord(y,x)

    def get_y_le(self,x):
        t_sp, h_st, t_st, t_sk, t_tb, taper = x
        return (x - self.x_lewing)/(0.25*self.chord_root*(1-taper)*2/self.span)

    def get_y_te(self,x):
        return (x - self.x_lewing - self.chord_root)/(-0.75*self.chord_root*(1-taper)*2/self.span)

    def get_x_start_wb(self,y,x):
        return self.get_x_le(y,x) + 0.15*self.chord(y,x)

    def get_x_end_wb(self,y,x):
        return self.get_x_te(y,x) -0.25*self.chord(y,x)

    def get_y_start_wb(self,x,input):
        t_sp, h_st, t_st, t_sk, t_tb, taper = input
        return (x-self.x_lewing-0.15*self.chord_root)/(0.1*self.chord_root*(1-taper)*2/self.span)

    def get_y_start_wb(self,x,input):
        t_sp, h_st, t_st, t_sk, t_tb, taper = input
        return (x-self.x_lewing-0.75*self.chord_root)/(-0.5*self.chord_root*(1-taper)*2/self.span)
    ...
    def get_r_o(self,x):
        t_sp, h_st, t_st, t_sk, t_tb, taper = x
        return self.height(self.y_rotor_loc[0],x)/2 - t_sk*3
    ...
    def I_xx(self, x):#TODO implement dissappearing stringers
        t_sp, h_st, t_st, t_sk, t_tb, taper = x
        h = self.height(self.y,x)
        # nst = n_st(c_r, b_st)
        Ist = self.I_st_x(h_st, t_st)
        Isp = self.I_sp_x(t_sp, self.y,x)
        A = self.get_area_str(h_st,t_st)
        return 2 * (Ist + A * (0.5 * h) ** 2) * self.n_str + 2 * Isp + 2 * (0.6 * self.chord(self.y,x) * t_sk ** 3 / 12 + t_sk * 0.6 * self.chord_root * (0.5 * h) ** 2)
    ...
    def I_zz(self, x):#TODO implement dissappearing stringers
        ...
        Ast = self.get_area_str(h_st,t_st)
        Asp = t_sp*self.w_sp(y,x)*2 + (h-2*t_sp)*t_sp
        centre_line = self.chord_root*0.25 + self.chord(y,x)*0.25
        ...
        moments_of_inertia += 2*(Isp + Asp * (self.chord(y,x)/2)*(self.chord(y,x)/2)) + 2* t_sk*self.chord(y,x)**3/12
...
#----------------Load functions--------------------
    ...
    def torque_from_tip(self,x):
        if self.hover_bool == True:
            ...
            distance_outboard = ((self.get_x_start_wb(self.span/2,x) + 0.5 * 0.6 * (self.chord(self.span/2,x)) - self.x_rotor_loc[2]))
            distance_inboard = ((self.get_x_start_wb(y[index],x) + 0.5 * 0.6 * (self.chord(y[index],x))) - self.x_rotor_loc[0])
            ...
            difference_array = np.absolute(y-self.y_rotor_loc[0])
            index = difference_array.argmin()
            torque_array = np.ones(len(y)) * (- self.engine_weight*9.81)*((self.get_x_start_wb(self.span/2) + 0.5 * 0.6 * (self.chord(self.span/2,x)) - self.x_rotor_loc[2])) #Torque at from tip roto
    ...
    def weight_from_tip(self, x):#TODO implement dissappearing stringers #TODO Include rib weights
        ...
        weight_skin = t_sk * ((0.6 * self.chord(self.span/2,x) + 0.6 * self.chord(y,x))* (self.span/2- y) / 2) * 2 + self.perimiter_ellipse(0.15*self.chord(y,x),self.height(y,x))*t_sk * 0.15 + np.sqrt((0.25*self.chord(y,x))**2 + (self.height(y,x))**2)*2*t_sk
        weight_spar_flanges = (self.w_sp(self.span/2,x) + self.w_sp(y,x))*(self.span/2- y)/2 * t_sp * self.density * 4
        weight_spar_web = (self.height(self.span/2,x) - 2*t_sp + self.height(y,x) - 2*t_sp) * (self.span/2- y) /2 * t_sp *self.density * 2
        ...
        weight_ribs = np.linspace(len(y),1,len(y)) * self.chord(y,x) * self.height(y,x) * self.t_rib * self.density
        #weight_ribs = np.sort(np.append(np.linspace(1,5,5),np.linspace(1,5,5))) * self.chord(y,x) * self.height(y,x) * self.t_rib * self.density
        #print(weight_ribs)
...
#-----------Stress functions---------
    def bending_stress_x_from_tip(self, x):
        return self.moment_x_from_tip(x)/self.I_xx(x) * self.height(self.y,x)/2
    ...
    def shear_flow_torque(self,x):
        ...
        t_sp, h_st, t_st, t_sk, t_tb, taper = x
        return T/(2*self.chord(y,x)*self.height(y,x))
...
#-------Own formulas-------
    ...
    def f_ult(self, h_st,t_st,t_sk,y):
        ...
        # n=n_st(c_r,b_st)
        A=self.n_str*A_st+0.6*self.chord(y,x)*t_sk
...
def GetWingWeight(wing: Wing, engine: Engine, material: Material, aero: Aero):
    ...
    #------SET BOUNDS--------
    height_tip = wingbox.height(wingbox.span/2,X) - 2*tsk#NOTE Set upper value so the stringer is not bigger than the wing itself.
    # print(height_tip)
    xlower =         1e-3,         1e-3,   1e-4, 1e-4,         1e-4, 0
    xupper = height_tip/2, height_tip/2, 3.3e-2, 1e-1, height_tip/2, 1
    ...
    resMin = minimize(wingbox.total_weight, resGA.X, method='SLSQP',
                ...
                options=options)
    wing.spar_thickness, wing.stringer_height, wing.stringer_thickness, wing.wingskin_thickness, wing.torsion_bar_thickness, wing.taper = resGA.X
