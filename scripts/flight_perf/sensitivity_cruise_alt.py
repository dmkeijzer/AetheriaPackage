...
for dict_name in dict_names:
    ...
    for i in range(300, max_cruise_alt, 1000):
        ...
        transition_simulation = numerical_simulation(y_start=30.5, mass=PerformanceClass.MTOM, g0=const.g0, S=WingClass.surface, CL_climb=AeroClass.cl_climb_clean,
                                    ...
                                    Adisk=const.diskarea, lod_climb=AeroClass.ld_climb, eff_climb=data['prop_eff'], v_stall=AeroClass.v_stall)
        ...
        P_loit_land = hoverstuffopen(PerformanceClass.MTOM*const.g0, const.rho_sl, PerformanceClass.MTOM/const.diskarea,data["TW"])[1]
        E_loit_vert = P_loit_land * 30 # 30 sec for hovering vertically
        # print('t', 30)

        #----------------------- Landing----------------------- 
        # print('----------- landing')
        # landing_power_var = hoverstuffopen(PerformanceClass.MTOM*const.g0, const.rho_sl, PerformanceClass.MTOM/const.diskarea,data["TW"])[1]
