...
#change in V
V_delta_var = V_delta(C_T=C_T_var, S_W=data['S'], n_e=4, D=diameter_propellers, V_0=const.v_cr)

# effective Diameter
D_star_var = D_star(D=diameter_propellers, V_0=const.v_cr, V_delta=V_delta_var)

A_eff_var = A_s_eff(b_W=data["b"], S_W=data['S'], n_e=4, D=diameter_propellers, V_0=const.v_cr, V_delta=V_delta_var)[0]

# DATCOM
CL_eff_alpha_var = CL_effective_alpha(mach=data["mach_cruise"], A_s_eff= A_eff_var, sweep_half=-data["sweep_le"])

# angles
angles = alpha_s(CL_wing=data["cL_cruise"], CL_alpha_s_eff=CL_eff_alpha_var, i_cs = i_cs_var, angle_of_attack= 0, alpha_0=data['alpha_zero_L'], V_0=const.v_cr, V_delta=V_delta_var, delta_alpha_zero_f=0)
...
prop_lift_var = prop_lift_thrust(T=drag_cruise, rho=rho_cr, V_0=const.v_cr, S_W=data['S'], angle_of_attack=i_cs_var)
