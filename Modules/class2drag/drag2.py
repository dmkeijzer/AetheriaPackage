...
class Fuselage():
    ...
    length: float = cockpit_parabol_length + \
        cylindrical_fuselage_length + conical_tail_length
    diameter: float = None
...
class componentdrag:
    def __init__(self, type, fuselage: Fuselage, wing1: Wing, wing2: Wing, airfoil: Airfoil, flightparam: FlightParam, winglet1: Winglet, winglet2: Winglet, S_ref, MAC, s1, s2, S_v, C_L_minD, height_between_wings, ):
        ...
        self.c = MAC  # mac
        self.b = np.sqrt(
            (0.5*(s1*wing1.aspectratio + s2*wing2.aspectratio))*S_ref)
        self.h_wl1 = winglet1.height  # winglet height 1
        self.h_wl2 = winglet2.height  # winglet height 2
        self.AR = 0.5*(s1*(wing1.aspectratio*winglet_factor(winglet1.height, np.sqrt(wing1.aspectratio*S_ref*s1), winglet1.smoothness)) +
                       s2*(wing2.aspectratio*winglet_factor(winglet2.height, np.sqrt(wing2.aspectratio*S_ref*s2), winglet2.smoothness)))
        ...
        self.S_t = 0.5*(1.4*wing1.chord_tip)*winglet1.height * \
            2 + 0.5*(1.4*wing2.chord_tip) * winglet2.height
        self.SweepLE = wing1  # sweep LE
        self.C_L_minD = C_L_minD / \
            (np.cos(self.SweepLE) ** 2)  # cl at minimum cd
